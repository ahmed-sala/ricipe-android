...
public interface UserDao {
    ...
    @Query("SELECT * FROM users WHERE pending_registration_sync = 1")
    Single<List<UserEntity>> getAllPendingRegistrationSyncUsers();
    @Query("UPDATE users SET pending_registration_sync = 0, pending_plain_password = NULL, " +
            "last_synced_at = :syncedAt, updated_at = :updatedAt WHERE id = :userId")
    Single<Integer> clearRegistrationSyncFlag(String userId, long syncedAt, long updatedAt);
}
